...
import { IMTableBodyRowProps } from './models';
import { Theme } from '@mui/material';

class MTableBodyRow extends Component<IMTableBodyRowProps> {
    renderColumns() {
    ...
    renderActions() {
        ...
        const width = (actions?.length ?? 0) * 42;

        return (
            <TableCell padding='none' key='key-actions-column' style={{ width, padding: '0px 5px', ...this.props.options.actionsCellStyle }} className={cellClassName}>
    ...
    renderSelectionColumn() {
        ...
        const cellClassName = this.props.options.fixedColumns ? 'cell-fixed' : '';
        const width = 42 + (9 * (this.props.treeDataMaxLevel - 1));
            ...
            return (
                <TableCell padding='none' key='key-detail-panel-column'>
                    <div style={{ width: 42 * this.props.detailPanel.length, textAlign: 'center', display: 'inline-block' }}>
                        {this.props.detailPanel.map((detailPanel: any, index: number) => {

                            const panel = typeof detailPanel === 'function' ? detailPanel(this.props.data) : detailPanel;
